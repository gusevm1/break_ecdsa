...
class SignServer(CommandServer):
    ...
    def initialize_new_round(self):
        ...
        if self.correct_signature:
            ...
            ecdsa_verify = self.ecdsa.Verify(self.pubkey, self.message, r, s)
            print(f"correct signature, ecdsa.Verify: {ecdsa_verify}")
            print(f"self.pubkey: {self.pubkey}, self.message: {self.message}, r: {r}, s: {s}")    
        else:
